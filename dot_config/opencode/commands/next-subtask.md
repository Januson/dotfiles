...
4. if there is no `In Progress` or `Pending` sub-task, stop and report that the plan has no remaining implementation
   work

Treat the selected sub-task entry as the full implementation brief. Implement only that sub-task, following its related
requirements, dependencies, in-scope and out-of-scope notes, risks, implementation suggestions, testing guidance, and
done-when criteria.

After implementation:

1. ask the `@executor` subagent to run the smallest relevant validation first
2. ask the `@reviewer` subagent to review the changes against the active sub-task and write feedback to
